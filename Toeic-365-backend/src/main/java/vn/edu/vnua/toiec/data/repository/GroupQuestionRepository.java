...
public interface GroupQuestionRepository extends JpaRepository<GroupQuestion, Integer> {
    ...
    @Query("select new vn.edu.vnua.toiec.presentation.model.GroupQuestionResponse(p.numberPart, g.id, g.title, g.groupQuestionDesc, g.groupQuestionImg, g.audio, g.paragraph) from Part p inner join p.groupQuestions g")
    List<GroupQuestionResponse> findNumberPartAndAllGroupQuestion();

    @Query("select new vn.edu.vnua.toiec.presentation.model.GroupQuestionResponse(g.title) from Part p inner join p.groupQuestions g where p.numberPart = ?1")
